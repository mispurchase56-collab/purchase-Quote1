...
def get_gspread_client():
    try:
        ...
        if not os.path.exists(CREDENTIALS_PATH):
            st.error(f"❌ Missing {CREDENTIALS_PATH} file!")
            return None
...
def load_gsheet(spreadsheet_id: str, sheet_name: str) -> pd.DataFrame:
    ...
    gid_map = {
        ...
        "Vendor Master": "1517409243",
        "Location Master": "1657803362",
        "Purchase Quotes": "475418295"
    }
...
def page_dashboard():
    ...
    if not df.empty:
        ...
        quote_totals = df.groupby("Quote No").agg(
            total_items_inc_gst=("Price Inc. GST", lambda x: pd.to_numeric(x, errors="coerce").sum()),
